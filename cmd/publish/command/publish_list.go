...
func (s *PublishListService) PublishList(req *publish.DouyinPublishListRequest) (vs []*publish.Video, err error) {
	videos, err := db.PublishList(s.ctx, req.UserId)
	if err != nil {
		return nil, err
	}

	vs, err = pack.PublishVideos(s.ctx, videos, &req.UserId)
	if err != nil {
		return nil, err
	}
