...
func UploadVideoFile(bucketName string, objectName string, reader io.Reader, objectsize int64) error {
	ctx := context.Background()
...
// UploadFile 上传文件（提供reader）至 minio
func UploadJpgFile(bucketName string, objectName string, reader io.Reader, objectsize int64) error {
	ctx := context.Background()
	n, err := minioClient.PutObject(ctx, bucketName, objectName, reader, objectsize, minio.PutObjectOptions{
		//ContentType: "application/octet-stream",
...
func GetFileUrl(bucketName string, fileName string, expires time.Duration) (*url.URL, error) {
	...
	reqParams.Set("response-content-disposition", "video/mp4")
